...
def createBoxRowsFromCSV(csvFilePath, DATA_PATH, limit=sys.maxint):
    ...
    with open(csvFilePath, 'r') as csvfile:
        ...
        for row in reader:
            # print "row: ", row
            # # row[1] ->x, row[2] ->y, row[3] ->w, row[4] ->h
            box = BBox()
            box.path = os.path.join(DATA_PATH, row[0]).replace("\\", "/")
            # print box.path
            box.left = float(row[1])
            box.top = float(row[2])
            box.right = float(row[1]) + float(row[3])
            box.bottom = float(row[2]) + float(row[4])
...
def getValidWithBBox(dataRows, boxRows):
    ...
    for rowIdx, dataRow in enumerate(dataRows):
        ...
        # all_y = []
        # for i in range(68):
        #     all_x.append(lmd_xy[i][0])
        #     all_y.append(lmd_xy[i][1])
        # x_min = min(all_x)
        # x_max = max(all_x)
        # y_min = min(all_y)
        # y_max = max(all_y)
        ...
        if top >= det_box.top and bot <= det_box.bottom and left >= det_box.left and right <= det_box.right:
            ...
            ''' expand det_box  10% larger on each side  '''
            if DEBUG:
                det_bbox = copy.deepcopy(det_box)
            else:
                det_bbox = det_box
            # s = 0.1  扩充人脸框宽高百分之20％　　
            # s = 0.15  扩充人脸框宽高百分之30％
            # s = 0.1
            height = det_box.bottom - det_box.top                   # 原人脸框高
            width = det_box.right - det_box.left                    # 原人脸框宽
            # 人脸框扩充
            det_bbox.top = det_box.top                              # 原始人脸框top
            det_bbox.bottom = det_box.bottom + 0.15 * height     # 向下扩充0.05
            det_bbox.left = det_box.left - 0.25 * width                # 向左扩充0.1
            det_bbox.right = det_box.right + 0.25 * width              # 向右扩充0.1
            # 去除人脸上半部分(额头)
            det_bbox.top = det_bbox.top + 0.15 * (height - width)    #top部分重新往下拉高减宽的0.8倍
            if det_bbox.top < 0:
                det_bbox.top = 0
            if det_bbox.left < 0:
                det_bbox.left = 0
            if det_bbox.bottom > dataRow.image.shape[0]:
                det_bbox.bottom = dataRow.image.shape[0]
            if det_bbox.right > dataRow.image.shape[1]:
                det_bbox.right = dataRow.image.shape[1]             #第一次根据人脸框外扩
            # 原人脸框扩充0.1后对应的人脸框
            # 加判断--如果有关键点在人脸扩充框之外 -- 设置/强行设置最小外接矩形
            # if det_bbox.left > x_min or det_bbox.top > y_min or det_bbox.right < x_max or det_bbox.bottom < y_max:
            #     # 有关键点在人脸扩充框之外
            #     if det_bbox.left > x_min:
            #         det_bbox.left = x_min - 0.07 * width            #根据最小关键点坐ｘ标再往外扩充0.05
            #         if det_bbox.left < 0:                           #边界判断
            #             det_bbox.left = 0
            #     if det_bbox.top > y_min:
            #         det_bbox.top = y_min - 0.08 * height        #防止top点正好在线上
            #         if det_bbox.top < 0:
            #             det_bbox.top = 0
            #     if det_bbox.right < x_max:
            #         det_bbox.right = x_max + 0.07 * width
            #         if det_bbox.right > dataRow.image.shape[1]:
            #             det_bbox.right = dataRow.image.shape[1]
            #     if det_bbox.bottom < y_max:
            #         det_bbox.bottom = y_max + 0.05 * height
            #         if det_bbox.bottom > dataRow.image.shape[0]:
            #             det_bbox.bottom = dataRow.image.shape[0]
            ...
            if DEBUG:
                img_draw = dataRow.image.copy()
                cv2.rectangle(img_draw, (int(det_bbox.left), int(det_bbox.top)),
                              (int(det_bbox.right), int(det_bbox.bottom)), (0, 255, 0), 1)
                cv2.rectangle(img_draw, (int(det_box.left), int(det_box.top)),
                              (int(det_box.right), int(det_box.bottom)), (0, 0, 255), 1)
                for k in range(68):
                    cv2.circle(img_draw, (dataRow.landmarks()[k * 2], dataRow.landmarks()[k * 2 + 1]), 1, (0, 255, 200))
                #cv2.imshow('src image', img_draw)
                cv2.imwrite('./src/%d.jpg' % (rowIdx), img_draw)
                #cv2.waitKey()
        else:
            R.outsideLandmarks += 1

    return validRow, R


def writeHD5(dataRows, outputPath, setTxtFilePATH, meanTrainSet, scaleTrainSet, IMAGE_SIZE, mirror=False):
    ...
    for dataRowOrig in dataRows:
        ...
        dataRow = dataRowOrig.copyCroppedByBBox(dataRowOrig.fbbox, siz=(
        IMAGE_SIZE, IMAGE_SIZE))  # Get a cropped scale copy of the data row
        if DEBUG:
            img_draw = dataRow.image.copy()
            for k in range(68):
                cv2.circle(img_draw, (dataRow.landmarks()[k * 2], dataRow.landmarks()[k * 2 + 1]), 1, (0, 255, 200))
            #cv2.imshow('resize image', img_draw)
            cv2.imwrite('./crop/%d.jpg' % (i), img_draw)
            #cv2.waitKey()

        scaledLM = dataRow.landmarksScaledMinus05_plus05()

        image = dataRow.image.astype('f4')
        image_chs = cv2.split(image)
        for k, img_ch in enumerate(image_chs):
            image_chs[k] = (img_ch - meanTrainSet[k]) * scaleTrainSet
        image = cv2.merge(image_chs)
        image = np.transpose(image, (2, 0, 1))  # (64,64,3) to (3,64,64)
        HD5Images[i, :] = image
        ...
        if mirror:
            dataRow = dataRowOrig.copyCroppedByBBox(dataRowOrig.fbbox, siz=(
            IMAGE_SIZE, IMAGE_SIZE)).copyMirrored()  # Get a cropped scale copy of the data row
            ...
            if DEBUG:
                ...
                for k in range(68):
                    cv2.circle(img_draw, (dataRow.landmarks()[k * 2], dataRow.landmarks()[k * 2 + 1]), 1, (0, 255, 200))
                #cv2.imshow('resize image', img_draw)
                cv2.imwrite('/home/data/landmark/network/data/debug/crop/mirror_%d.jpg' % (i), img_draw)
                #cv2.waitKey()
            ...
            for k, img_ch in enumerate(image_chs):
                image_chs[k] = (img_ch - meanTrainSet[k]) * scaleTrainSet
            image = cv2.merge(image_chs)
            image = np.transpose(image, (2, 0, 1))  # (40,40,3) to (3,40,40)
            HD5Images[i, :] = image
            HD5Landmarks[i, :] = scaledLM
            i += 1

    '''shuffle dataset and landmarks '''
    idx_list = np.arange(BATCH_SIZE)
    random.shuffle(idx_list)
    shuffled_HD5Images = HD5Images[idx_list, :]
    shuffled_HD5Landmarks = HD5Landmarks[idx_list, :]

    with h5py.File(outputPath, 'w') as T:
        T.create_dataset("X", data = shuffled_HD5Images)
        T.create_dataset("landmarks", data = shuffled_HD5Landmarks)
...
class ErrorAcum:  # Used to count error per landmark
    ...
    def __repr__(self):
        return '%f mean error, %d items, %d failures  %f precent' % (
        self.meanError().mean() * 100, self.itemsCounter, self.failureCounter,
        float(self.failureCounter) / self.itemsCounter if self.itemsCounter > 0 else 0)
...
class Angle:    #pitch yaw roll

    def __init__(self, pitch=0, yaw=0, roll=0, path=''):
        self.pitch = pitch
        self.yaw = yaw
        self.roll = roll
        self.path = path
...
class BBox:  # Bounding box
    ...
    def __repr__(self):
        return 'left(%.1f), top(%.1f), right(%.1f), bottom(%.1f) w(%d) h(%d)' % (
...
class DataRow:
    ...
    IMAGE_SIZE = NORM_IMAGE_SIZE  # 64

    def __init__(self, path='', p1=(0, 0,), p2=(0, 0), p3=(0, 0), p4=(0, 0), p5=(0, 0), p6=(0, 0,), p7=(0, 0),
                 p8=(0, 0), p9=(0, 0), p10=(0, 0),
                 p11=(0, 0,), p12=(0, 0), p13=(0, 0), p14=(0, 0), p15=(0, 0), p16=(0, 0,), p17=(0, 0), p18=(0, 0),
                 p19=(0, 0), p20=(0, 0),
                 p21=(0, 0,), p22=(0, 0), p23=(0, 0), p24=(0, 0), p25=(0, 0), p26=(0, 0,), p27=(0, 0), p28=(0, 0),
                 p29=(0, 0), p30=(0, 0),
                 p31=(0, 0,), p32=(0, 0), p33=(0, 0), p34=(0, 0), p35=(0, 0), p36=(0, 0,), p37=(0, 0), p38=(0, 0),
                 p39=(0, 0), p40=(0, 0),
                 p41=(0, 0,), p42=(0, 0), p43=(0, 0), p44=(0, 0), p45=(0, 0), p46=(0, 0,), p47=(0, 0), p48=(0, 0),
                 p49=(0, 0), p50=(0, 0),
                 p51=(0, 0,), p52=(0, 0), p53=(0, 0), p54=(0, 0), p55=(0, 0), p56=(0, 0,), p57=(0, 0), p58=(0, 0),
                 p59=(0, 0), p60=(0, 0),
    ...
    def landmarksScaledMinus05_plus05(self):
        # return numpy float array with ordered values
        return self.landmarks().astype('f4') / self.IMAGE_SIZE - 0.5
    ...
    def DataRowFromNameBoxInterlaved(row, root=''):  # lfw_5590 + net_7876 (interleaved)
        ...
        d.p1 = (float(row[5]), float(row[6]))
        d.p2 = (float(row[7]), float(row[8]))
        d.p3 = (float(row[9]), float(row[10]))
        d.p4 = (float(row[11]), float(row[12]))
        d.p5 = (float(row[13]), float(row[14]))
        d.p6 = (float(row[15]), float(row[16]))
        d.p7 = (float(row[17]), float(row[18]))
        d.p8 = (float(row[19]), float(row[20]))
        d.p9 = (float(row[21]), float(row[22]))
        d.p10 = (float(row[23]), float(row[24]))
        d.p11 = (float(row[25]), float(row[26]))
        d.p12 = (float(row[27]), float(row[28]))
        d.p13 = (float(row[29]), float(row[30]))
        d.p14 = (float(row[31]), float(row[32]))
        d.p15 = (float(row[33]), float(row[34]))
        d.p16 = (float(row[35]), float(row[36]))
        d.p17 = (float(row[37]), float(row[38]))
        d.p18 = (float(row[39]), float(row[40]))
        d.p19 = (float(row[41]), float(row[42]))
        d.p20 = (float(row[43]), float(row[44]))
        d.p21 = (float(row[45]), float(row[46]))
        d.p22 = (float(row[47]), float(row[48]))
        d.p23 = (float(row[49]), float(row[50]))
        d.p24 = (float(row[51]), float(row[52]))
        d.p25 = (float(row[53]), float(row[54]))
        d.p26 = (float(row[55]), float(row[56]))
        d.p27 = (float(row[57]), float(row[58]))
        d.p28 = (float(row[59]), float(row[60]))
        d.p29 = (float(row[61]), float(row[62]))
        d.p30 = (float(row[63]), float(row[64]))
        d.p31 = (float(row[65]), float(row[66]))
        d.p32 = (float(row[67]), float(row[68]))
        d.p33 = (float(row[69]), float(row[70]))
        d.p34 = (float(row[71]), float(row[72]))
        d.p35 = (float(row[73]), float(row[74]))
        d.p36 = (float(row[75]), float(row[76]))
        d.p37 = (float(row[77]), float(row[78]))
        d.p38 = (float(row[79]), float(row[80]))
        d.p39 = (float(row[81]), float(row[82]))
        d.p40 = (float(row[83]), float(row[84]))
        d.p41 = (float(row[85]), float(row[86]))
        d.p42 = (float(row[87]), float(row[88]))
        d.p43 = (float(row[89]), float(row[90]))
        d.p44 = (float(row[91]), float(row[92]))
        d.p45 = (float(row[93]), float(row[94]))
        d.p46 = (float(row[95]), float(row[96]))
        d.p47 = (float(row[97]), float(row[98]))
        d.p48 = (float(row[99]), float(row[100]))
        d.p49 = (float(row[101]), float(row[102]))
        d.p50 = (float(row[103]), float(row[104]))
        d.p51 = (float(row[105]), float(row[106]))
        d.p52 = (float(row[107]), float(row[108]))
        d.p53 = (float(row[109]), float(row[110]))
        d.p54 = (float(row[111]), float(row[112]))
        d.p55 = (float(row[113]), float(row[114]))
        d.p56 = (float(row[115]), float(row[116]))
        d.p57 = (float(row[117]), float(row[118]))
        d.p58 = (float(row[119]), float(row[120]))
        d.p59 = (float(row[121]), float(row[122]))
        d.p60 = (float(row[123]), float(row[124]))
        d.p61 = (float(row[125]), float(row[126]))
        d.p62 = (float(row[127]), float(row[128]))
        d.p63 = (float(row[129]), float(row[130]))
        d.p64 = (float(row[131]), float(row[132]))
        d.p65 = (float(row[133]), float(row[134]))
        d.p66 = (float(row[135]), float(row[136]))
        d.p67 = (float(row[137]), float(row[138]))
        d.p68 = (float(row[139]), float(row[140]))
    ...
    def drawLandmarks(self, r=3, color=255, other=None, title=None):
        ...
        if hasattr(self, 'fbbox'):
            ...
            cv2.rectangle(M, (int(self.fbbox.left), int(self.fbbox.top)),
                          (int(self.fbbox.right), int(self.fbbox.bottom)), color, 2)
            ...
            cv2.rectangle(M, (int(det_bbox.left), int(det_bbox.top)), (int(det_bbox.right), int(det_bbox.bottom)),
                          (0, 200, 0), 2)
        return M
    ...
    def copyCroppedByBBox(self, fbbox, siz):
        """
        @ fbbox : BBox
        Returns a copy with cropped, scaled to size
        """
        siz = np.array(siz, dtype=np.float)
